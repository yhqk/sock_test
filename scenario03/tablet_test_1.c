/* tablet_test_1.c
  ...
  Use case 1: 
   ...
   2) Several Messages with dummy from response as zero.  
        - USCTL_GET_VERSION
        - USCTL_SET_ACQU_PARAMS
        - USCTL_SET_MOTOR_PARAMS
        - USCTL_SPI_READ (unclear how many of them tablet sends)
        - USCTL_SPI_READ
        - USCTL_SPI_READ
        - USCTL_START_ACQ (UDP IP address and port passed to scanner)
    
   3) UDP server is openned and starting to receive images, which
      is a collection of 10 image files made on 24th of September 2015. 
     
   4) Following message sequence will repeat 3 times
      - receiving 35 seconds UDP data from scanner
      - USCTL_SET_MOTOR_PARAMS
      - USCTL_STOP_ACQ
      - pause for 5 seconds
      - USCTL_START_ACQ 
     
   5) - USCTL_SET_MOTOR_PARAMS
      - USCTL_STOP_ACQ
   User can either rerun the tablet_test_1 again or stop testing also 
   from scanner side. 
  
  * Compiling and Execution *
  $ gcc -o exec_c tablet_test_1.c -Wall -lpthread
...
int main(int argc, char *argv[])
{
    ...
    /* ===> step (4) <=== */
    for ( i=0; i<3; i++ ) {
        printf("\n ===> No %d: receiving 35 sec UDP data before stopping scanner <===\n", i+1);         
        sleep(35); /* UDP thread is sending data */
        tablet_dummy_in_handler((struct sockaddr *)&tcpServAddr, USCTL_SET_MOTOR_PARAMS);    
        tablet_dummy_in_handler((struct sockaddr *)&tcpServAddr, USCTL_STOP_ACQ);
        sleep(5); /* client start to view scanning image again after 5 sec */
        udpServAddr.sin_family = AF_INET;
        udpServAddr.sin_addr.s_addr = inet_addr(argv[3]);
        udpServAddr.sin_port = htons(udpPortNo);        
        tablet_startAcqIn_handler((struct sockaddr *)&tcpServAddr, (struct sockaddr *)&udpServAddr);
